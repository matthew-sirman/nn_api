...
namespace nnet {
	analytics::analytics()
		: analytics(log_output_type::LOG_PER_EPOCH)
	{
	...
	void analytics::on_epoch_start()
	{
		...
		if (log_type == LOG_PER_EPOCH && v != VERBOSITY_LOW) {
	...
	void analytics::on_step_start()
	{
		...
		if (log_type == LOG_PER_STEPS && cur_step % log_steps == 0) {
			tmr.start();
		}
	}
	...
	void analytics::on_epoch_end(float avg_loss)
	{
		...
		if (log_type == log_output_type::LOG_PER_EPOCH && v != VERBOSITY_LOW) {
			//stop the period timer
			tmr.stop();

			//print each property if it is specified within the flag
			if (log_props & log_property::LOG_STEP)
				printf("Epoch %d complete\n", cur_epoch + 1);
			if (log_props & log_property::LOG_AVG_LOSS)
				printf("Average Loss: %e\n", avg_loss);
			if (log_props & log_property::LOG_TIME_STAMP)
	...
	void analytics::on_step_end(float avg_loss)
	{
		...
		if (log_type == log_output_type::LOG_PER_STEPS && v != VERBOSITY_LOW && cur_step % log_steps == log_steps - 1) {
			//stop the period timer
			tmr.stop();
