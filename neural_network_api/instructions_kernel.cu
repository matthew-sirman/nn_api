...
namespace nnet {
	namespace nnet_internal {
		...
		//kernel function to round an array of numbers given a probability
		__global__ void d_gen_dropout_array(float* in, float* out, float p, size_t size) {
			//get the thread id
			int id = threadIdx.x + blockDim.x * blockIdx.x;

			//if in range, write to output
			if (id < size) {
				if (in[id] < p)
					out[id] = 1.0;
				else
		...
		//kernel to cast an array of integers to an array of floats
		__global__ void d_cast_int_to_float(int* input, float* output, size_t size) {
			//get the id
			int id = threadIdx.x + blockDim.x * blockIdx.x;
		...
		{
			curandGenerator_t prng = global_prng();
		...
		void random_dropout_array(float* d_array_p, float p, size_t size)
		{
			...
			//declare a pointer placeholder
			float* d_raw_arr;
	
			//allocate memory to pointer
			cuda_safe_call(cudaMallocManaged(&d_raw_arr, sizeof(float) * size));

			//generate numbers between 0 and 1
			curandGenerateUniform(prng, d_raw_arr, size);
			...
			d_gen_dropout_array<<<blocks_per_grid, threads_per_block>>>(d_raw_arr, d_array_p, p, size);

			//free the placeholder
			cuda_safe_call(cudaFree(d_raw_arr));
		}
		...
		void average_vector(float * d_matrix, float * d_output_p, size_t size, size_t batch_size, int divisor)
		{
			...
			if (batch_size <= 2) {
				...
				dim3 threads_per_block(2, 1);
				d_average_vector<2><<<blocks_per_grid, threads_per_block>>>(d_matrix, d_output_p, size, batch_size, divisor);
			}
			else if (batch_size <= 8) {
				dim3 threads_per_block(4, 1);
				d_average_vector<4><<<blocks_per_grid, threads_per_block>>>(d_matrix, d_output_p, size, batch_size, divisor);
			}
			else if (batch_size <= 16) {
				dim3 threads_per_block(8, 1);
				d_average_vector<8><<<blocks_per_grid, threads_per_block>>>(d_matrix, d_output_p, size, batch_size, divisor);
			}
			else if (batch_size <= 32) {
				dim3 threads_per_block(16, 1);
				d_average_vector<16><<<blocks_per_grid, threads_per_block>>>(d_matrix, d_output_p, size, batch_size, divisor);
			}
			else if (batch_size <= 64) {
				dim3 threads_per_block(32, 1);
				d_average_vector<32><<<blocks_per_grid, threads_per_block>>>(d_matrix, d_output_p, size, batch_size, divisor);
			}
			else if (batch_size <= 128) {
				dim3 threads_per_block(64, 1);
				d_average_vector<64><<<blocks_per_grid, threads_per_block>>>(d_matrix, d_output_p, size, batch_size, divisor);
			}
			else if (batch_size <= 256) {
				dim3 threads_per_block(128, 1);
				d_average_vector<128><<<blocks_per_grid, threads_per_block>>>(d_matrix, d_output_p, size, batch_size, divisor);
			}
			else if (batch_size <= 512) {
				dim3 threads_per_block(256, 1);
				d_average_vector<256><<<blocks_per_grid, threads_per_block>>>(d_matrix, d_output_p, size, batch_size, divisor);
			}
			else if (batch_size <= 1024) {
				dim3 threads_per_block(512, 1);
				d_average_vector<512><<<blocks_per_grid, threads_per_block>>>(d_matrix, d_output_p, size, batch_size, divisor);
			}
		...
		void softmax_cross_entropy_cost(float * d_input_p, float * d_target_p, float * d_output_p, size_t size, size_t batch_size)
		{
			...
			if (size <= 2) {
				...
				d_softmax_cross_entropy<1><<<blocks_per_grid, threads_per_block>>>(d_input_p, d_target_p, d_output_p, size, batch_size);
			}
			else if (size <= 4) {
				threads_per_block.x = 2;
				d_softmax_cross_entropy<2><<<blocks_per_grid, threads_per_block>>>(d_input_p, d_target_p, d_output_p, size, batch_size);
			}
			else if (size <= 8) {
				threads_per_block.x = 4;
				d_softmax_cross_entropy<4><<<blocks_per_grid, threads_per_block>>>(d_input_p, d_target_p, d_output_p, size, batch_size);
			}
			else if (size <= 16) {
				threads_per_block.x = 8;
				d_softmax_cross_entropy<8><<<blocks_per_grid, threads_per_block>>>(d_input_p, d_target_p, d_output_p, size, batch_size);
			}
			else if (size <= 32) {
				threads_per_block.x = 16;
				d_softmax_cross_entropy<16><<<blocks_per_grid, threads_per_block>>>(d_input_p, d_target_p, d_output_p, size, batch_size);
			}
			else if (size <= 64) {
				threads_per_block.x = 32;
				d_softmax_cross_entropy<32><<<blocks_per_grid, threads_per_block>>>(d_input_p, d_target_p, d_output_p, size, batch_size);
			}
			else if (size <= 128) {
				threads_per_block.x = 64;
				d_softmax_cross_entropy<64><<<blocks_per_grid, threads_per_block>>>(d_input_p, d_target_p, d_output_p, size, batch_size);
			}
			else if (size <= 256) {
				threads_per_block.x = 128;
				d_softmax_cross_entropy<128><<<blocks_per_grid, threads_per_block>>>(d_input_p, d_target_p, d_output_p, size, batch_size);
			}
			else if (size <= 512) {
				threads_per_block.x = 256;
				d_softmax_cross_entropy<256><<<blocks_per_grid, threads_per_block>>>(d_input_p, d_target_p, d_output_p, size, batch_size);
			}
			else if (size <= 1024) {
				threads_per_block.x = 512;
				d_softmax_cross_entropy<512><<<blocks_per_grid, threads_per_block>>>(d_input_p, d_target_p, d_output_p, size, batch_size);
			}
			else if (size <= 2048) {
				threads_per_block.x = 1024;
				d_softmax_cross_entropy<1024><<<blocks_per_grid, threads_per_block>>>(d_input_p, d_target_p, d_output_p, size, batch_size);
			}
		}
		...
		void apply_argmax(float * d_input, int * d_output, size_t input_size, size_t batch_size)
		{
			...
			cuda_safe_call(cudaMallocManaged(&d_max_vals, sizeof(float) * batch_size));

			apply_argmax(d_input, d_max_vals, d_output, input_size, batch_size);
		...
		void apply_argmax(float * d_input, float * d_max_vals, int * d_output, size_t input_size, size_t batch_size)
		{
			...
			fill_device_array(d_max_vals, FLOAT_MIN, batch_size);

			//setup block and grid sizes
			dim3 threads_per_block(BLOCK_SIZE, 1);
			dim3 blocks_per_grid(ceil_div(BLOCK_SIZE * 2, input_size), batch_size);
		...
		void cast_int_to_float(int* input, float* output, size_t size)
{
	...
	//launch the comparison equal kernel
	d_cast_int_to_float<<<blocks_per_grid, threads_per_block>>>(input, output, size);
}
	}
}