...
namespace nnet {
	...
	struct __align__(8) d_matrix {
		...
		//the device
		inline d_matrix<T>(size_t n_size, size_t m_size, T * d_data) {
			this->n_size = n_size;
			this->m_size = m_size;
			this->d_data = d_data;
		}
	...
	inline void create_device_matrix(d_matrix<m_T>& matrix, size_t n_size, size_t m_size, m_T* d_data) {
	...
	inline void create_f1_matrix(d_matrix<float1>* matrix, size_t n_size, size_t m_size, float* data) {
